<?php
/*
Plugin Name: Widget für aktuelle globale Beiträge
Plugin URI: https://n3rds.work/piestingtal_source/multisite-beitragsindex-plugin/
Description: Zeige die neuesten globalen Beiträge in einem Widget an
Author: WMS N@W
...
$MyUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	'https://n3rds.work//wp-update-server/?action=get_metadata&slug=beitragsindex-widget', 
	__FILE__, 
	'beitragsindex-widget' 
);
...
class Recent_Global_Posts_Widget extends WP_Widget {
	...
	public function __construct( $widget_options = array(), $control_options = array() ) {
		$widget_options = array_merge( array(
			'classname'   => 'rgpwidget',
			'description' => __( 'Aktuelle globale Beiträge', 'rgpwidget' ),
			), $widget_options );
		...
		parent::__construct( 'rgpwidget', __( 'Aktuelle globale Beiträge', 'rgpwidget' ), $widget_options, $control_options );
	}
	...
	public function widget( $args, $instance ) {
		...
		$title = !empty( $instance['recentglobalpoststitle'] ) ? $instance['recentglobalpoststitle'] : __( 'Aktuelle globale Beiträge' );
					...
					echo '<li>';
						...
						switch ( $recentglobalpostsdisplay ) {
							case self::DISPLAY_BLOG_CONTENT:
								...
								echo '<br><a href="', $the_permalink, '">', __( 'Weiterlesen', 'rgpwidget' ), ' &raquo;</a>';
								break;
	...
	public function form( $instance ) {
		...
		$displays = array(
			self::DISPLAY_TITLE_CONTENT      => __( 'Titel und Inhalt', 'rgpwidget' ),
			self::DISPLAY_TITLE_BLOG_CONTENT => __( 'Titel, Blogname und Inhalt', 'rgpwidget' ),
			self::DISPLAY_TITLE              => __( 'Nur Titel', 'rgpwidget' ),
			self::DISPLAY_TITLE_BLOG         => __( 'Titel und Blogname', 'rgpwidget' ),
			self::DISPLAY_CONTENT            => __( 'Nur Inhalt', 'rgpwidget' ),
			self::DISPLAY_BLOG_CONTENT       => __( 'Blogname und Inhalt', 'rgpwidget' ),
		);
		...
		?><p>
			<label for="<?php echo $this->get_field_id( 'recentglobalpoststitle' ) ?>"><?php _e( 'Titel', 'rgpwidget' ) ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'recentglobalpostsdisplay' ) ?>"><?php _e( 'Anzeige', 'rgpwidget' ) ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'recentglobalpostsnumber' ) ?>"><?php _e( 'Nummer', 'rgpwidget' ) ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'recentglobalpoststitlecharacters' ) ?>"><?php _e( 'Titelzeichen', 'rgpwidget' ); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'recentglobalpostscontentcharacters' ) ?>"><?php _e( 'Inhaltszeichen', 'rgpwidget' ); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'recentglobalpostsavatars' ) ?>"><?php _e( 'Avatare', 'rgpwidget' ) ?>:</label>
			<select id="<?php echo $this->get_field_id( 'recentglobalpostsavatars' ) ?>" class="widefat" name="<?php echo $this->get_field_name( 'recentglobalpostsavatars' ) ?>">
				<option value="show"<?php selected( $instance['recentglobalpostsavatars'], 'show' ) ?> ><?php _e( 'Anzeigen', 'rgpwidget' ) ?></option>
				<option value="hide"<?php selected( $instance['recentglobalpostsavatars'], 'hide' ) ?> ><?php _e( 'Verbergen', 'rgpwidget' ) ?></option>
			</select>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'recentglobalpostsavatarsize' ) ?>"><?php _e( 'Avatar-Größe', 'rgpwidget' ) ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'recentglobalpoststype' ) ?>"><?php _e( 'Beitrag-Typ', 'rgpwidget' ) ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'exclude_blogs' ) ?>"><?php _e( 'Blogs ausschließen', 'rgpwidget' ) ?>:</label>
			...
			<small><?php esc_html_e( 'Blog-IDs, durch Kommas getrennt.', 'rgpwidget' ) ?></small>
		</p>
